...
async function main() {
  try {
    ...
    console.log('🚀 Starting EURC to USDC EXACT IN swap with multiple protocols...')
    console.log('📊 User wants to swap exactly 0.1 EURC to USDC')
    console.log('🔄 Using protocols: Soroswap, Phoenix, Aqua')
    ...
    const quoteRequest = {
        ...
        amount: BigInt(1000000), // 0.1 EURC in stroops (7 decimals) - EXACT IN
        tradeType: TradeType.EXACT_IN,
        protocols: [SupportedProtocols.SOROSWAP, SupportedProtocols.PHOENIX, SupportedProtocols.AQUA], // Multiple protocols
    ...
    // Here, because is a Soroban tx, we need to:
    // 1. Sign the transaction with the user
    // 2. Wrap the signed tx with a FeeBumpTransaction
    // 3. Sign the  new tx by the sponsor
    // 4. Submit the transaction to the network
    
    // 1 Sign the transaction with the user
    console.log('')
    console.log('1. The user signs the transaction...')
    swapTransaction.sign(userKeypair)
    
    console.log('')
    console.log('2. We wrap the signed tx with a FeeBumpTransaction...')
    ...
    console.log('')
    console.log('3. The sponsor signs the feeBumpTransaction...')
    feeBumpTransaction.sign(sponsorKeypair)
    
    console.log('')
    console.log('The transaction XDR:', feeBumpTransaction.toXDR())
    ...
    if (balancesBefore && balancesAfter) {
      ...
      if (usdcChange > 0 && Math.abs(eurcChange) > 0) {
        const exchangeRate = usdcChange / Math.abs(eurcChange)
        console.log(`💱 Effective Exchange Rate: ${exchangeRate.toFixed(6)} USDC per EURC`)
      }
    }
