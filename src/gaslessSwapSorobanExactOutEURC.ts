...
async function main() {
  try {
    ...
    console.log('🚀 Starting USDC to EURC EXACT OUT swap with Soroban protocols...')
    console.log('📊 User wants to receive exactly 0.1 EURC for USDC')
    ...
    const quoteRequest = {
        ...
        amount: BigInt(1000000), // 0.1 EURC in stroops (7 decimals) - EXACT OUT
        tradeType: TradeType.EXACT_OUT,
        protocols: [SupportedProtocols.SOROSWAP, SupportedProtocols.PHOENIX, SupportedProtocols.AQUA], // Soroban protocols
    ...
    if (balancesBefore && balancesAfter) {
      ...
      if (Math.abs(usdcChange) > 0 && eurcChange > 0) {
        const exchangeRate = Math.abs(usdcChange) / eurcChange
